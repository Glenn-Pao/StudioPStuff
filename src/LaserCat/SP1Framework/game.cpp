...
void render()
{
	...
	//render score
	//moves the score down if it's out of vision range
	...
	//render health
	//moves the health down if it's out of vision range
	if (charLocation.Y == 24)
	{
		gotoXY(0, 1);
		colour(0x1B);
...
void paw (double &count, double &count2, double &count3, claw &enemy)
{
	...
	//increase paws' speed as game progress
	if (elapsedTime <= 10)
	{
...
void pickup(double &count, double &count2, double &count3, item &bonus)
{
	...
	if (check == 0)
	{
		if (bonus.ID == 1 && count >= 75 && elapsedTime >= 15)
		{
			...
			//prevents item from spawning in cat
			do {
				redo[0] = 0;
				...
				for (int i = 0; i < 10; ++i)
				{
					for (int j = 0; j < 10; ++j)
					{
						if (bonus.loc.X == deadspace[2].loc.X +i && bonus.loc.Y == deadspace[2].loc.Y+j)
						{
							redo[0] = 1;
							continue;
						}
	...
	else
	{
		if (bonus.ID == 1 && count >= 75 && elapsedTime >= 46)
		{
			...
			for (int i = 0; i < 10; ++i)
			{
				for (int j = 0; j < 10; ++j)
				{
					if ((bonus.loc.X == deadspace[0].loc.X + i && bonus.loc.Y == deadspace[0].loc.Y + j) || (bonus.loc.X == deadspace[1].loc.X + i && bonus.loc.Y == deadspace[1].loc.Y + j))
					{
						redo[3] = 1;
	...
	//moves item back to default loc if it's picked up
